package com.matteopelucco.bugjump;
 ...
 * 
 * A bug is walking slowly and trying to catch stars. It jumps to collect them when the player hits the space bar. For each non yellow star, it gets 5 points. For each yellow star, a random number of
 * multicolored stars is generated. For each yellow star, the bug gets 10 points, for each other color, it gets 5 points.
 * 
 * Note that if you want to write a serious game, it is not advisable to model it after this one. For instance, there a native way in JavaFX to animate, and the timing mechanism used for this game is
 * not precise and causes the animation to be choppy.
 * 
 * This game was originally written to demonstrate rxScala, a reactive streams library. The original version is very interesting in its own right. You can check it out at
...
public class Game extends javafx.application.Application {
	...
	/* Game parameters */
	static final String RES_DIR = "file:///" + System.getProperty("user.dir") + "/src/main/resources";
	...
	/**
	 * Accumulator for collected points. one of only 3 mutable (non final) variables declared in this game.
	 */
	...
	public void start(final Stage stage) {
		...
		/* Game loop */
		/*
		 * We pass the game logic as a lambda parameter to a method that will execute it periodically.
		 */
		gameLoop(() -> {
			/*
			 * Lambda expression passed, it implements the logic of method run() of functional interface Runnable.
			 */
			Platform.runLater(() -> {
	...
	private void checkCollision(final Collectible sun, final Bug bug,
		...
		/*
		 * If a collision has occurred and the bug is still jumping, do not check for the collision.
		 */
		if (bug.collided)
			return;
	 ...
	 * 
	 * @param gameLogic
	 *            Lambda expression of the game logic.
	 */
	...
	/**
	 * Use of optional to indicate that this method either returns some color or no color.
	 * 
	 * @return
	 */
	private static Optional<Color> nextColor() {
		...
		if (index == 2)
			...
			return Optional.of(colors[index]);
	}
	...
	private static class Grass implements Entity {
		...
		public Grass(final StackPane root) {
			/*
			 * We change the stream to a list so we can repeatedly operate on the tiles. Note that the tiles themselves are still mutable.
			 * 
			 * The use of constructing tiles like this is that we can parallelize tile creation just by modifying to .range(0, nrTiles).parallel() Note that, if we added each tile directly to root
			 * using root.getChildren().add(this) when we use parallel(), we get an exception, this is because collection storing children in root is not thread-safe. This is one example that shows
			 * why you need to be very careful when you mutate variables outside the lambda expression, and why you should always strive to minimize mutability.
			 */
			tiles = IntStream
					.range(0, nrTiles)
	...
	static class Bonus {
					...
					.mapToObj(i -> new BImage() {
						{
							...
							/*
							 * Here you see how we operate on an optional type, to extract a value. Note that since the absence of effect is expressed by having a null effect, we are forced to assign
							 * a null eventually to ColorAdjust. The Java API still does not employ optional to indicate the absence of value, which is unfortunate.
							 */
		...
		public int getBonusPoints() {
			/*
			 * Use of map and reduce to calculate the points collected. Note how by using reduce, we avoid the need of having an explicit variable in our code to accumulate the points. Often times,
			 * using stream operations helps to reduce mutability in code.
			 */
			return bonus.stream()
		...
		public void update() {
			if (isJumping) {
				...
				if (position == homeY) {
					...
					collided = false;
					/*
					 * When the bug hits the ground again after a jump, we remove the bonus images from teh scene. We use filter method of streams to do so.
					 */
